...
<div id="contenido">
    <?php
        if (isset($_POST['MostrarStock']) && isset($_POST['familia']) && !$msg){
            ...
            try {
                $result=$conex->prepare("SELECT producto.cod, producto.PVP as precio, producto.nombre_corto FROM producto WHERE producto.familia = :familia");
                $result->bindParam(":familia", $_POST['familia']);
                $result->execute();
                while ($datos=$result->fetch()){
                    echo "<br> Nombre: $datos->nombre_corto: $datos->precio euros";
                    echo "<a href='editar.php?cod=" . $datos->cod . "'><button>Editar</button></a>";                
                }
            } catch(Exception $ex){
